...
#  获取远程港澳台直播源文件
#url = "https://raw.githubusercontent.com/frxz751113/AAAAA/main/IPTV/TWW.txt"          #源采集地址
#r = requests.get(url)
...
if __name__ == "__main__":
    source_file_path = 'IPTV/TWW.txt'  # 替换为你的源文件路径
    output_file_path = 'TWW'  # 替换为你的输出文件路径,不要后缀名
    ...
    for i, line in tqdm(enumerate(lines), total=total_lines, desc="Processing", unit='line'):
        ...
        if len(parts) == 2:
            ...
            if ip_key and ip_key in detected_ips:
                ...
                cap.release()
                # 根据捕获的帧数判断状态并记录结果
                if frame_count >= 220:  #10秒内超过230帧则写入
